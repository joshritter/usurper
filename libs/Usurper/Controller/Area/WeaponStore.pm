...
sub printItemList {
    ...
    my $n = 0;
    use Data::Dumper;
    foreach my $weapon_id (sort { $weapons->{$a}->getAttribute('cost') <=> $weapons->{$b}->getAttribute('cost') } keys %$weapons){
        my $weapon = $weapons->{$weapon_id};
        my $prefix = $n+1 . ".) ". $weapon->getName();
        #variable whitespace to keep things aligned
        print "$prefix ".$self->generateWhiteSpace(41-length($prefix)).$self->formatNumber($weapon->getAttribute('cost')) ."\n\r";
...
sub buyItem {
    ...
    my $store = $self->{'_store'};
    my $weapons = $store->getWeapons();
    my @weapons = sort { $a->getAttribute('cost') <=> $b->getAttribute('cost') } values %$weapons;
